...
function handleError(error) {
    if (error.isBoom) {
        ...
        // the reason for `error.statusCode` is because there might be error
        // that comes from other lambda function (during lambda-invoke) and
        // this error is in json format so if it contains statusCode then
        // we can assume it was created by Boom library
